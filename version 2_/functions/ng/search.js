 ...
 * @returns Search result for guild, player, and faction
 */
module.exports = async (query) => {
    let queryString = `?name=${query}`;

    let res = await get(`https://api.ngmc.co/v1/search${queryString}`, { headers: { "Authorization": client.ngKey } }).catch(console.log)
    if (!res) return { code: 400, msg: "Unable to fetch the search results" };
    if (res.status >= 400) {
        if (res.status == 404) return { code: 404, msg: `No Search results found for: ${query}` };
