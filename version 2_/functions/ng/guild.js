 ...
 * @returns {object} Data for a Guild in NetherGames
*/
module.exports = async (guild, expand, withStats, withVoteStatus, withFactionData, withPunishments, withWarnings) => {
    let queryString = '?a=0';
    ...
    let res = await get(`https://api.ngmc.co/v1/guilds/${guild}${queryString}`, { headers: { "Authorization": client.ngKey } }).catch(console.log)
    if (!res) return { code: 400, msg: "Unable to fetch the guild" };
    if (res.status >= 400) {
        if (res.status == 404) return { code: 404, msg: `<:erroro:1001073206389649408> Guild not found: ${guild}` };
