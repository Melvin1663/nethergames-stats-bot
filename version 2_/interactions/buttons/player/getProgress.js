module.exports = async (json, client, Discord, int) => {
    ...
    let res = await require('../../../commands_src/nethergames/progress')(Discord, client, int, [json.data])
    if (!res.content) res.content = `Requested by ${int.user.tag}`;

    int.editReply(res).catch(console.log);

    let comp = int.message.components[2].components.filter(v => v.customId.includes('getProgress'))
