...
module.exports = async (Discord, client, msg, args) => {
  try {
    ...
    let player = await require("../../functions/ng/player")(
      require("../../functions/deQ")(args.join(" ")).replace(/ +/g, "%20"),{ expand: false, withFactionData: false, withGuildData: false, withOnline: true, withPunishments: false, withSkinData: false, withStats: true, withVoteStatus: false });
    if (player.code != 0) {
      if (player.code == 404) {
    ...
    let res = require("../../functions/getWLR")(player);
